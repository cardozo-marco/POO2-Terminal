...
public class CriterioMenorCantidadTerminalesTest {
	
	private CriterioMenorCantidadTerminales criterio;
	private TerminalPortuaria origen;
	private TerminalPortuaria destino;
	
	@BeforeEach
	public void setUp() {
		criterio = new CriterioMenorCantidadTerminales();
		origen = mock(TerminalPortuaria.class);
		destino = mock(TerminalPortuaria.class);
	}
	
	@Test
	public void buscarMejorCircuitoPorCantidadTerminalesTest() {
		...
		when(circuito1.cantidadTerminalesEntre(origen, destino)).thenReturn(3);
		when(circuito2.cantidadTerminalesEntre(origen, destino)).thenReturn(1);
		when(circuito3.cantidadTerminalesEntre(origen, destino)).thenReturn(2);
